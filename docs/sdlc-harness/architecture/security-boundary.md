...
安全边界是 OpenBitFun Agent 执行层的安全域。它判断当前动作是否可能越权、泄露、破坏环境或被恶意上下文操控，并输出允许、沙箱允许、询问、应急放行、拒绝或策略拒绝。
...
安全边界不按工具名称、命令关键词或模型判断结果直接授权。凡经 OpenBitFun owner/facade 发起的 tool、MCP、skill、
...
沙箱不是单一能力。OpenBitFun 需要同时区分三类保护：
