...
本文只说明 SDLC Harness 如何消费 OpenBitFun 的 OpenCode 扩展能力，不另建一套插件格式、运行时或权限体系。
...
这意味着 Harness 只依赖 OpenBitFun 的稳定工具、事件、权限和诊断接口，不读取 JS 进程句柄、OpenCode 原始载荷或
插件运行时内部状态。
...
OpenCode Hook 是可变换接口，不应统一降级为“建议”或“只读候选”。适配器负责顺序和参数转换，对应 OpenBitFun
...
- 用户可以显式导入支持的配置字段到 OpenBitFun；导入前显示直接使用、需要转换和会降级的项目。
...
本地默认兼容策略允许插件使用当前用户通常可用的文件、网络、进程和环境能力。经 OpenBitFun 能力接口的调用可以
