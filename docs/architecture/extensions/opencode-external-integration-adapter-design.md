...
本文定义 OpenCode 开发工具包、Server、ACP、IDE、Web、GitHub、GitLab 和 Slack 如何与 OpenBitFun 对接。它们是
...
目标是复用 OpenBitFun 已有会话、工具、工作区和远程能力，为真实外部调用方提供必要兼容接口，并明确原始 OpenCode
客户端能否直接连接。
...
flowchart LR
  ...
  Protocol["入口专用协议适配器"]
  Transport["OpenBitFun 能力服务 / 事件投影"]
...
- 普通 OpenBitFun 启动不因项目 `server` 配置改变监听地址。
...
- 对外写入方法不得返回伪成功；认证、工作区和权限判断由 OpenBitFun 归属模块执行。
...
2. 复用 OpenCode 配置兼容结果和插件能力，但任务由 OpenBitFun Runtime 执行。
3. 分支、提交、PR/MR、Issue、评论或 Slack thread 由对应连接器负责，失败可重试且不重复回写。
4. 文档明确这是 OpenBitFun 集成还是原始 OpenCode 包直连，不能混用兼容结论。
...
6. 原始客户端直连与 OpenBitFun 原生替代分别标记，不能用同类结果代替协议兼容测试。

当前不承诺原始 OpenCode Web/attach、GitHub Action、GitLab runner 或 Slack 包直接连接 OpenBitFun；这些限制需要随
