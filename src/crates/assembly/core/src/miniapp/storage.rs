...
pub use openbitfun_product_domains::miniapp::storage::MiniAppImportBundleWriteRequest;
use openbitfun_services_integrations::miniapp::storage::{
...
impl MiniAppStorage {
    ...
    pub async fn ensure_app_dir(&self, app_id: &str) -> OpenBitFunResult<()> {
        self.inner
    ...
    pub async fn list_app_ids(&self) -> OpenBitFunResult<Vec<String>> {
        self.inner.list_app_ids().await.map_err(map_storage_error)
    }

    pub async fn load(&self, app_id: &str) -> OpenBitFunResult<MiniApp> {
        self.inner.load(app_id).await.map_err(map_storage_error)
    }

    pub async fn load_meta(&self, app_id: &str) -> OpenBitFunResult<MiniAppMeta> {
        self.inner
            .load_meta(app_id)
            .await
            .map_err(map_storage_error)
    ...
    pub async fn load_source_only(&self, app_id: &str) -> OpenBitFunResult<MiniAppSource> {
        self.inner
    ...
    pub async fn save(&self, app: &MiniApp) -> OpenBitFunResult<()> {
    ...
    pub async fn load_draft_app(&self, app_id: &str, draft_id: &str) -> OpenBitFunResult<MiniApp> {
        self.inner
    ...
    pub async fn delete_draft(&self, app_id: &str, draft_id: &str) -> OpenBitFunResult<()> {
        self.inner
    ...
    pub async fn load_app_storage(&self, app_id: &str) -> OpenBitFunResult<serde_json::Value> {
        self.inner
    ...
    pub async fn load_version(&self, app_id: &str, version: u32) -> OpenBitFunResult<MiniApp> {
        self.inner
    ...
    pub async fn read_import_meta_json(&self, source_path: &Path) -> OpenBitFunResult<String> {
        self.inner
...
fn map_storage_error(error: MiniAppStorageError) -> OpenBitFunError {
    match error.kind() {
        MiniAppStorageErrorKind::NotFound => OpenBitFunError::NotFound(error.message().to_string()),
        ...
        MiniAppStorageErrorKind::Deserialization => {
            OpenBitFunError::parse(error.message().to_string())
        }
        MiniAppStorageErrorKind::Io => OpenBitFunError::io(error.message().to_string()),
        MiniAppStorageErrorKind::Backend => OpenBitFunError::service(error.message().to_string()),
    }
}
...
fn map_miniapp_port_error(error: OpenBitFunError) -> MiniAppPortError {
    let kind = match &error {
        ...
        OpenBitFunError::Deserialization(_) => MiniAppPortErrorKind::Deserialization,
