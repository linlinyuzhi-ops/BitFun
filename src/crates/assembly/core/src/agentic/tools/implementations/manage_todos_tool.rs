...
impl ManageTodosTool {
    ...
    fn resolve_workspace_ref(context: &ToolUseContext) -> OpenBitFunResult<CronWorkspaceRef> {
    ...
    fn parse_iso_timestamp_ms(value: &str, field: &str) -> OpenBitFunResult<i64> {
    ...
    ) -> OpenBitFunResult<CronJobTarget> {
        match target {
            TodoTargetInput::Session { session_id } => {
                ...
                openbitfun_core_types::validate_session_id(&session_id).map_err(OpenBitFunError::tool)?;
    ...
    fn serialize_job(job: &CronJob) -> OpenBitFunResult<Value> {
        serde_json::to_value(job).map_err(|error| OpenBitFunError::serialization(error.to_string()))
    }
}
...
impl TodoScheduleInput {
    fn to_service_schedule(&self) -> OpenBitFunResult<CronSchedule> {
        match self {
            Self::At { at } => {
                let at = at.trim();
                if at.is_empty() {
                    return Err(OpenBitFunError::tool("schedule.at cannot be empty".to_string()));
                }
            ...
            Self::Every {
                ...
                let every_ms = every_seconds.checked_mul(1_000).ok_or_else(|| {
                    OpenBitFunError::tool("schedule.everySeconds is too large".to_string())
                })?;
            ...
            Self::Cron { expr, tz } => {
                let expr = expr.trim();
                if expr.is_empty() {
                    return Err(OpenBitFunError::tool("schedule.expr cannot be empty".to_string()));
                }
    ...
    async fn call_impl(
        ...
        match parsed.action {
            ManageTodosAction::Create => {
                let text = parsed
                    .text
                    .ok_or_else(|| OpenBitFunError::tool("text is required for create".to_string()))?;
                if text.trim().is_empty() {
                    return Err(OpenBitFunError::tool("text must not be empty".to_string()));
                }
                let schedule = parsed.schedule.ok_or_else(|| {
                    OpenBitFunError::tool("schedule is required for create".to_string())
                })?;
                let target = parsed
                    .target
                    .ok_or_else(|| OpenBitFunError::tool("target is required for create".to_string()))?;
            ...
            ManageTodosAction::Update => {
                let job_id = parsed
                    .job_id
                    .as_deref()
                    .ok_or_else(|| OpenBitFunError::tool("jobId is required for update".to_string()))?;
                if job_id.trim().is_empty() {
                    return Err(OpenBitFunError::tool("jobId cannot be empty".to_string()));
                }
            ...
            ManageTodosAction::Delete => {
                let job_id = parsed
                    .job_id
                    .as_deref()
                    .ok_or_else(|| OpenBitFunError::tool("jobId is required for delete".to_string()))?;
                if job_id.trim().is_empty() {
                    return Err(OpenBitFunError::tool("jobId cannot be empty".to_string()));
                }
