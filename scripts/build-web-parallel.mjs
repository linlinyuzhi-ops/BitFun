 ...
 * esbuild), so `BITFUN_SKIP_AUDITS=1` local packaging drops it.
 */
const skipAudits = ['1', 'true', 'yes'].includes(
  String(process.env.BITFUN_SKIP_AUDITS ?? '').toLowerCase()
);
...
if (skipAudits) {
  process.stdout.write(
    '[build-web-parallel] BITFUN_SKIP_AUDITS=1: skipping tsc type-check (does not affect dist)\n'
  );
} else {
  parallelSteps.push([
    'type-check',
